...
if __name__ == "__main__":
    ...
    try:
        kafka_producer = connect_to_kafka(args.server)
    except NoBrokersAvailable as e:
        print(f"No brokers available on '{args.server}'")
        kafka_producer = None

    stream_df_to_kafka(producer=kafka_producer, topic=args.topic, key=key, df=df_data)
